...
const My_notes_top_bar = () => {
    const { sortBy, setSortBy, localSearch, setLocalSearch, fetchSearch, includeTags, setIncludeTags, excludeTags, setExcludeTags, defaultType } = useMyNotesContext();
        ...
        <div className='w-full bg-white shadow-md py-3'>
            ...
            <div className='flex flex-row items-center px-4 md:px-5 mt-1.5 gap-3 justify-between'>
                <div className='flex flex-row items-center gap-3'>
                    <PiNotebookLight size={38} className='opacity-70' />
                    <p className='font-["Julius_Sans_One"] text-[18px] md:text-[32px] text-[#3E4A34] font-thin select-none'>
                        {defaultType === 'post' ? 'MY NOTES' : defaultType === 'discussion' ? 'MY FORUMS' : 'MY CONTENT'}
                    </p>
                </div>
            </div>
            ...
            <div className='mx-4 md:mx-5 mt-2 flex flex-row gap-3 items-center border-2 rounded-2xl p-1.5 pl-4'>
                ...
                <input
                    type='text'
                    placeholder='Search your content by title, subject, or tags...'

    ...
    <div className='w-full bg-white shadow-md py-3'>
      ...
      <div className='flex flex-row items-center ml-4 md:ml-5 mt-1.5 gap-3 justify-between w-full pr-4 md:pr-8'>
        <div className='flex flex-row items-center gap-3'>
          ...
          <p className='font-["Julius_Sans_One"] text-[18px] md:text-[32px] font-thin text-[#3E4A34] select-none'>HOME</p>
        </div>
      </div>
          ...
          <div className='flex flex-row flex-wrap gap-1.5'>
            {SORT_OPTIONS.map(({ key, icon, label }) => (
              <button
                key={key}
                onClick={() => setSortBy(key)}
                className={`flex flex-row items-center gap-1 px-3 py-1.5 rounded-full transition-all text-[12px] font-medium ${sortBy === key ? 'bg-[#3E4A34] text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                  }`}
              >
                {icon}
                {label}
